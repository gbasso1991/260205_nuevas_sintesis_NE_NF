...
class ResultadosESAR:
    ...
    def __init__(self, directorio_analisis):
        ...
        # 2. Detectar si la temperatura es significativa
        self._detectar_temperatura_significativa()
        
        # 3. Crear índices según si la temperatura es significativa o no
        self._crear_indices()
        
        # 4. Cargar ciclos de magnetización extremos
        self._cargar_ciclos_extremos()
        
        # 5. Calcular estadísticas básicas
        self._calcular_estadisticas()
        
        # 6. Mostrar resumen de lo cargado
        self._mostrar_resumen()
        
        # 7. Mostrar ciclos cargados
    ...
    def _cargar_resultados_txt(self):
        ...
        try:
            ...
            # Convertir temperatura a float si no lo es
            if self.temperatura is not None:
                self.temperatura = np.array(self.temperatura, dtype=float)
    ...
    def _detectar_temperatura_significativa(self):
        """
        Detecta si la temperatura es significativa o si es constante.
        
        Establece:
        - self.temperatura_significativa (bool): True si hay variación real
        - self.temperatura_constante (float o None): Valor si es constante
        """
        if self.temperatura is None or len(self.temperatura) == 0:
            self.temperatura_significativa = False
            self.temperatura_constante = None
            print("⚠ No hay datos de temperatura")
            return
        
        # Calcular estadísticas de temperatura
        temp_std = np.std(self.temperatura)
        temp_mean = np.mean(self.temperatura)
        temp_min = np.min(self.temperatura)
        temp_max = np.max(self.temperatura)
        
        # Umbral para considerar variación significativa (ajustable)
        # Usamos 0.1°C como umbral para detectar cambios reales
        UMBRAL_VARIACION = 0.1
        
        if temp_std < UMBRAL_VARIACION:
            self.temperatura_significativa = False
            self.temperatura_constante = temp_mean
            
            # Verificar si es exactamente el valor por defecto (20.0)
            if np.abs(temp_mean - 20.0) < 0.01:
                self.temperatura_origen = 'por_defecto'
                print(f"✓ Temperatura constante: {temp_mean:.1f}°C (valor por defecto)")
            else:
                self.temperatura_origen = 'constante_medida'
                print(f"✓ Temperatura constante: {temp_mean:.1f}°C (medida real)")
        else:
            self.temperatura_significativa = True
            self.temperatura_constante = None
            self.temperatura_origen = 'variable'
            print(f"✓ Temperatura variable: {temp_min:.1f}°C a {temp_max:.1f}°C (σ={temp_std:.2f}°C)")
    
    def _crear_indices(self):
        """
        Crea índices de organización según el tipo de datos de temperatura.
        
        Establece:
        - self.indices (np.array): Array de índices para organizar los datos
        - self.indice_principal (str): 'temperatura' o 'indice' según qué usar
        """
        n_datos = len(self.files)
        
        if self.temperatura_significativa:
            # Si la temperatura es significativa, organizar por temperatura
            self.indice_principal = 'temperatura'
            
            # Crear índice basado en temperatura (orden ascendente)
            indices_ordenados = np.argsort(self.temperatura)
            self.indices = indices_ordenados
            
            # Crear también un array de índice secuencial
            self.indice_secuencial = np.arange(n_datos)
            
            print("✓ Organización: por temperatura (ascendente)")
        else:
            # Si la temperatura no es significativa, organizar por índice secuencial
            self.indice_principal = 'indice'
            
            # Índice secuencial
            self.indice_secuencial = np.arange(n_datos)
            self.indices = self.indice_secuencial.copy()
            
            # Si tenemos tiempo, lo usamos como referencia
            if hasattr(self, 'time') and len(self.time) > 0:
                self.indice_temporal = self.time.copy()
                print("✓ Organización: por tiempo de medición")
            else:
                self.indice_temporal = None
                print("✓ Organización: por índice secuencial")
    ...
    def _mostrar_resumen(self):
        ...
        # Información de temperatura (modificada)
        if hasattr(self, 'temperatura_significativa'):
            if self.temperatura_significativa:
                temp_min = np.min(self.temperatura)
                temp_max = np.max(self.temperatura)
                print(f"✓ Temperatura significativa: {temp_min:.1f}°C - {temp_max:.1f}°C")
                print(f"  Organización: por temperatura (ascendente)")
            else:
                print(f"✗ Temperatura constante: {self.temperatura_constante:.1f}°C")
                if self.temperatura_origen == 'por_defecto':
                    print(f"  (Valor por defecto - no hubo registro de temperatura)")
                print(f"  Organización: por {'tiempo' if hasattr(self, 'time') and len(self.time) > 0 else 'índice'}")
    ...
    def plot_evolucion_temporal(self, figsize=(12, 10), guardar=False):
        ...
        # Título adaptado según el tipo de datos
        if self.temperatura_significativa:
            titulo = 'Evolución temporal de parámetros (T variable)'
        else:
            titulo = 'Evolución temporal de parámetros (T constante)'
...
# ==================== EVOLUCIÓN VS TEMPERATURA ====================
    def plot_evolucion_temperatura(self, figsize=(12, 8), guardar=False):
        ...
        # Si la temperatura no es significativa, mostrar mensaje y sugerir plot_evolucion_temporal
        if not self.temperatura_significativa:
            print(f"⚠ La temperatura es constante ({self.temperatura_constante:.1f}°C)")
            print(f"  No tiene sentido graficar evolución vs temperatura.")
            print(f"  Usa plot_evolucion_temporal() para ver la evolución en el tiempo.")
            return None, None
        ...
        # Obtener datos ORDENADOS por temperatura (usando self.indices)
        temp_vals = self.temperatura[self.indices]
        
        # Encontrar la longitud mínima para todos los arrays
        min_len = min(len(temp_vals), 
                     len(self.tau[self.indices]), 
                     len(self.SAR[self.indices]), 
                     len(self.Hc[self.indices]), 
                     len(self.Mr[self.indices]))
        
        # Tomar solo los primeros min_len elementos ordenados
        temp_vals = temp_vals[:min_len]
        
        for idx, (titulo, attr, unidad, color) in enumerate(parametros_config):
            ax = axs[idx // 2, idx % 2]
            
            # Obtener datos ordenados por temperatura
            datos = getattr(self, attr)[self.indices][:min_len]
        ...
        return fig, axs
    def plot_evolucion_organizada(self, figsize=(12, 10), guardar=False):
        """
        Grafica la evolución de parámetros usando el índice apropiado.
        Decide automáticamente si usar temperatura o tiempo como eje X.
        ...
        fig, axs : matplotlib figure and axes array
        """
        # Determinar qué índice usar para el eje X
        if self.temperatura_significativa:
            eje_x = self.temperatura[self.indices]
            label_x = 'Temperatura (°C)'
            titulo_base = 'Evolución vs Temperatura'
        elif hasattr(self, 'time') and len(self.time) > 0:
            eje_x = self.time[self.indices]
            label_x = 'Tiempo (s)'
            titulo_base = 'Evolución Temporal'
        else:
            eje_x = self.indice_secuencial
            label_x = 'Índice de medición'
            titulo_base = 'Evolución por Mediciones'
        
        fig, axs = plt.subplots(3, 2, figsize=figsize, constrained_layout=True, sharex=True)
        fig.suptitle(f'{titulo_base} de parámetros', fontsize=16, fontweight='bold')
        ...
        datos_organizados = {}
        for _, attr, _, _ in parametros:
            if hasattr(self, attr):
                datos = getattr(self, attr)
                if len(datos) == len(self.indices):
                    datos_organizados[attr] = datos[self.indices]
                else:
                    datos_organizados[attr] = datos
        
        for idx, (titulo, attr, unidad, color) in enumerate(parametros):
            ax = axs[idx // 2, idx % 2]
            
            if attr in datos_organizados and len(datos_organizados[attr]) > 0:
                datos = datos_organizados[attr]
                
                # Graficar datos
                ax.plot(eje_x[:len(datos)], datos, 'o-', color=color, markersize=5,
                        label=f'{titulo}')
                
                # Agregar estadísticas si están disponibles
                if hasattr(self, 'estadisticas') and attr in self.estadisticas:
                    stats = self.estadisticas[attr]
                    media_str = f"{stats['media']:.3f}"
                    std_str = f"{stats['desviacion']:.3f}"
                    ax.text(0.05, 0.95, 
                           f"μ = {media_str} {unidad}\nσ = {std_str} {unidad}",
                           transform=ax.transAxes, fontsize=9,
                           verticalalignment='top',
                           bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
        ...
        if guardar:
            if isinstance(guardar, str):
                ...
                nombre_base = os.path.basename(self.directorio)
                nombre_archivo = f"evolucion_organizada.png"
    ...
    def obtener_datos_organizados(self, *atributos):
        """
        Devuelve los datos organizados según el índice principal.
        
        Parámetros:
        -----------
        *atributos : str
            Nombres de los atributos a obtener (ej: 'SAR', 'Hc', 'temperatura')
        
        Retorna:
        --------
        datos_organizados : dict o np.array
            Si se pide un atributo: array organizado
            Si se piden varios: dict con arrays organizados
        """
        if len(atributos) == 1:
            attr = atributos[0]
            if hasattr(self, attr):
                datos = getattr(self, attr)
                if len(datos) == len(self.indices):
                    return datos[self.indices]
                else:
                    return datos
            else:
                raise AttributeError(f"Atributo '{attr}' no encontrado")
            ...
            for attr in atributos:
                if hasattr(self, attr):
                    datos = getattr(self, attr)
                    if len(datos) == len(self.indices):
                        resultado[attr] = datos[self.indices]
                    else:
                        resultado[attr] = datos
                else:
                    raise AttributeError(f"Atributo '{attr}' no encontrado")
            return resultado
    ...
    def __str__(self):
        ...
        # Información de temperatura mejorada
        if hasattr(self, 'temperatura_significativa'):
            if self.temperatura_significativa:
                temp_min = self.temperatura.min()
                temp_max = self.temperatura.max()
                output += f"Temperatura: Variable ({temp_min:.1f}°C a {temp_max:.1f}°C)\n"
                output += f"Organización: Por temperatura ascendente\n"
            else:
                output += f"Temperatura: Constante ({self.temperatura_constante:.1f}°C)\n"
                if self.temperatura_origen == 'por_defecto':
                    output += f"  (Valor por defecto)\n"
                output += f"Organización: Por {'tiempo' if hasattr(self, 'time') and len(self.time) > 0 else 'índice'}\n"
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
class ResultadosESAR_repeticiones:
